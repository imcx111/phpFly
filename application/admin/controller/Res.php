...
class Res extends Admin {
    ...
    function _initialize() {
        parent::_initialize();
        $this->res_dir = APP_DIR . '/uploads/' . APP_THEME . '/';
        $this->res_http = APP_URL . '/uploads/' . APP_THEME . '/';
        //设置默认的文件大小
        ...
        if (empty($image_ext) && empty($flash_ext) && empty($media_ext) && empty($file_ext)) {
            $this->ext_arr = [
                ...
                'file' => ['mp3', 'mp4', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'txt', 'zip', 'rar', 'gz', 'bz2'],
            ];
        } else {
            $this->ext_arr = [
    ...
    public function iframe() {
        // 加载内容
        $this->assign('iframe_src', url('admin/res/index'));
        // 加载分类
        $parent = [
            'id' => 'image',
            'name' => '所有类型',
        ...
        if ($files) {
            ...
            if ($count) {
                ...
                if (input('get.directory')) {
                    ...
                    $this->success('', '', res_http($folder_this . '/' . input('get.directory') . '/' . $info->getSaveName()));
                } else {

                    $this->success('', '', res_http($folder_this . '/' . $info->getSaveName()));
                }
            }
        } else {
            $this->error('没有上传任何文件');
    ...
    public function index_folder() {
        ...
        if (empty($folder)) {
            ...
            $folder = str_replace(['../', '..\\', '..'], '', get_basename(html_entity_decode($folder)));
    ...
    public function _get_list($folder = 'image', $action = '') {
        ...
        foreach ($images as $image) {
            $name = str_split(get_basename($image), 255);
            if (is_dir($image)) {
                $url = '';
                $tmp = filemtime($image);
                ...
                if ($folder == 'image') {
                    $thumb = img_resize(mb_substr($image, mb_strlen($this->res_dir)), 300, 0);
                } else {
