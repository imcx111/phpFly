...
class WxPay {
     ...
     * @var string
     */
//    const APPID = 'wx22e1343f81b87a41';
//    const MCHID = '1488217372';
//    const KEY = '74f6b51e0b76569c1f12c09d90b456ae';
//    const APPSECRET = 'e8c69d00abb90cf8a73f82c10dc5fc8d';
    ...
    public function __construct($config = array()) {
        $this->config = array(
            ...
            'mch_id' => Config::get('weixin.mch_id'),
            'pay_apikey' => Config::get('weixin.pay_apikey'),
    ...
    public function wxpay($openid, $total_fee, $body, $order_sn) {
        ...
        $unifiedorder = array(
            ...
            'notify_url' => config('weixin.notify_url'), // 支付回调
            'trade_type' => 'JSAPI',
            'openid' => $openid
        );
        ...
        if (!$res) {
            return array('code' => 1, 'msg' => "Can't connect the server");
        }
        // 这句file_put_contents是用来查看服务器返回的结果 测试完可以删除了
        file_put_contents(APP_DIR . '/wxpay-error-info.txt', $res, FILE_APPEND);
        file_put_contents(APP_DIR . '/wxpay-sign-info.txt', json_encode($unifiedorder), FILE_APPEND);

        $content = self::xml2array($res);
        if (!isset($content['result_code'])) {
            return array('code' => 1, 'msg' => json_encode($content));
        }

        if (strval($content['result_code']) == 'FAIL') {
            return array('code' => 1, 'msg' => strval($content['err_code']) . ':' . strval($content['err_code_des']));
        }
        if (strval($content['return_code']) == 'FAIL') {
            return array('code' => 1, 'msg' => strval($content['return_msg']));
        }
...
//---------------------------------------------------------------用到的函数------------------------------------------------------
     ...
     * 供前台支付调用
     * @param string $openid    openid
     * @param real $total_fee   金额 0.01 
     * @param string $order_sn  订单号
     * @return type
     */
    public function pay_first($openid, $total_fee, $order_sn, $body = '') {
